...
export class MailController {
  ...
  async send(@Body() body: any) {
    ...
    try {
      ...
      // build CTA url to frontend checkout with sale id
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173'
      const ctaUrl = `${frontendUrl}/checkout?saleId=${sale.id}`

      const mailPayload = { ...body, saleId: sale.id, ctaUrl }
      const res = await this.mailService.sendContact(mailPayload)
      return { ok: true, saleId: sale.id, companyMessageId: res?.company ?? null, requesterMessageId: res?.requester ?? null }
    } catch (err) {
